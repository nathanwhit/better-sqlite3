...
void Backup::JS_new(const v8 ::FunctionCallbackInfo<v8 ::Value> &info) {
  UseAddon;
...
void Backup::JS_transfer(const v8 ::FunctionCallbackInfo<v8 ::Value> &info) {
  Backup *backup = Unwrap<Backup>(info.This());
  REQUIRE_ARGUMENT_INT32(FIRST_ARG, int pages);
...
void Backup::JS_close(const v8 ::FunctionCallbackInfo<v8 ::Value> &info) {
  Backup *backup = Unwrap<Backup>(info.This());
  assert(backup->db->GetState()->busy == false);
  if (backup->alive)
